...
const extensionListShowInStart = computed<ExtensionInfo[]>(() => {
  if (isPC) {
    const extensionList = currentExtensionList.value.filter(
...
const extensionListShowInEnd = computed<ExtensionInfo[]>(() => {
  if (isPC) {
    const searchExtension = currentExtensionList.value.find(
      (extension) => extension?.data?.name === "search"
    );
...
const onExtensionClick = (extension: ExtensionInfo) => {
  switch (extension?.data?.name) {
    ...
    case "videoCall":
      ...
      break;
    default:
      (extension as any)?.listener?.onClicked(
        currentConversation.value?._conversation
      );
      break;
  }
};
