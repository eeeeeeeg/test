...
import markImportantIcon from "../../../../assets/icon/msg-mark.svg";
import cancelmarkImportantIcon from "../../../../assets/icon/msg-removemark.svg";
...
const actionItems = ref([
  ...
  },
  {
    key: "markImportant",
    text: "标记重点",
    iconUrl: markImportantIcon,
    renderCondition() {
      if (!featureConfig.MarkImportant || !message.value) return false;
      return (
        isPC &&
    ...
    clickEvent: markImportantMessage,
  },
  {
    key: "cancelmarkImportant",
    text: "取消标记",
    iconUrl: cancelmarkImportantIcon,
    renderCondition() {
      if (!featureConfig.MarkImportant || !message.value) return false;
      return (
        isPC &&
    ...
    clickEvent: cancelmarkImportantMessage,
  },
  {
    key: "copy",
 ...
 *
 */
// async function markImportantMessage() {
//   if (!message.value) return;
//   const messageModel = TUIStore.getMessageModel(message.value.ID);
//   if (messageModel) {
//     const modifyData = messageModel;
//     modifyData.payload.text = "2444444modifyMessage";
//     try {
//       const a = await messageModel.modifyMessage(modifyData);
//       console.log("modifyMessage result:", a);
//     } catch (error) {
//       console.error("Failed to modify message:", error);
//     }
//   }
// }
function markImportantMessage() {
  if (!message.value) return;
  const messageModel = TUIStore.getMessageModel(message.value.ID);
  const modifdata = messageModel;
  modifdata.payload.text = "modifyMessage";
  let promise = messageModel.modifyMessage(modifdata);
  promise.catch((error) => {
    console.error("----", error);
 ...
 *
 */
function cancelmarkImportantMessage() {
  console.log("cancelmarkImportantMessage1：", TIM);
}
